...
class GenericFilterTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.gen_filter = GenericFilter(input="A",output="B", operation=EXAMPLE_OP)

    def test_empty_stream(self):
        # Testing a single execute call on an empty input Stream closes the output as well
        self.input_stream.close()
        self.gen_filter.execute([self.input_stream],[self.output_stream],None)
    ...
    def test_simple_stream_applies(self):
        ...
        while not self.output_stream.is_closed():
            self.gen_filter.execute([source_stream],[self.output_stream],None)
