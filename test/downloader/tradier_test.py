...
class TradierTimeseriesAdapterTest(unittest.TestCase):
    ...
    TICKERS = ['AAPL', 'MSFT']
    ATOM_KEYS = ['open', 'close', 'high', 'low', 'price', 'datetime', 'volume', 'vwap']
    ...
    def test_generic_timeseries(self):
        ...
        o_stream = adapter.download(tickers=self.TICKERS, interval="1min", start=start_dt, end=end_dt) # Assert doesn't raise exception
