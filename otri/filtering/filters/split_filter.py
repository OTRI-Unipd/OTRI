...
class SplitFilter(Filter):
    '''
    Splits a Queue based on the value range of a numerical field in the atoms.

    Inputs:
        A single queue.
    Outputs:
        Multiple Queues, as many as the possible value ranges are, plus one for the
        ...
        and so on. If atoms that do not have the given key are not to be ignored, the queue
    ...
    def __init__(self, inputs: str, outputs: Sequence[str], key: Any, ranges: Sequence, none_keys_output: str = None, side: str = 'left'):
        ...
        Parameters:
            input : str
                A single queue name.
            output : str
                The output queues names. Must have the same length as the number of ranges + 1.
                The contents of those queues depends on the 'side':
            ...
            ranges : Sequence
                ...
                The ouput queues will be N+1 or N+2 : less than r1, more than rn, the n-1 in-betweens,
    ...
    def setup(self, inputs: Sequence[Queue], outputs: Sequence[Queue], state: Mapping[str, Any]):
        ...
        if len(outputs) != out_count:
            raise AttributeError("SplitFilter requires {} output queues, {} given".format(
                out_count, len(outputs)))

    def _on_data(self, data, index):
        '''
        Puts the atom into the appropriate output queue.
        ...
        if self.__key in data.keys():
            # Find the appropriate Queue for the item.
...
class SwitchFilter(Filter):
    '''
    Splits a Queue based on the value of a field in the atoms.

    Inputs:
        A single queue.
    Outputs:
        Multiple Queues, as many as the requested cases (N), plus one for the default case,
        and one for the atoms that do not have the requested key, if enabled.
        Output queues of index 0 to N-1 will be the cases, N will be the default, N+1 will be
    ...
    def __init__(self, inputs: str, cases_outputs: Sequence[str], default_output: str, key: Any, cases: Set, none_keys_output: str = None):
        ...
        Parameters:
            input : str
                A single queue name.
            cases_output : str
                The output queues names that will contain data which data[key] equals to one of the cases.
            default_output : str
                The output queue name that will contain data which data[key] doesn't fall into any of the cases.
            key : Any
                The key on which to split values on.
            cases : Set
                The N values for which to split. Must be different values.
                The ouput queues will be N+1 or N+2 : the N cases, the default, and optionally one for
    ...
    def _on_data(self, data, index):
        ...
        '''
        if self.__key in data.keys():
            # Put the atom in the appropriate output queue.
